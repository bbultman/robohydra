var url   = require('url'),
    ...
    qs    = require('qs'),
    mime  = require('mime'),
    ProxyingAgent = require("proxying-agent").ProxyingAgent;
...
(function () {
    ...
    function proxyRequest(req, res, proxyTo, opts) {
        ...
        var directProxyConfiguration = (proxyUrl.protocol === 'https:') ?
                process.env.https_proxy : process.env.http_proxy;

        var requestOptions = {host: proxyToHost,
                              port: proxyToPort,
                              method: req.method,
                              path: proxyToPath,
                              headers: headers};

        if (directProxyConfiguration) {
            var parsedProxyConf = url.parse(directProxyConfiguration);
            console.log("XXX Proxy: " + parsedProxyConf.protocol +
                        "//" + parsedProxyConf.host);
            requestOptions.agent = new ProxyingAgent({
                proxy: directProxyConfiguration,
                tunnel: (proxyUrl.protocol === 'https:')
            });
        }

        console.log("XXX Proxied request:", requestOptions);
