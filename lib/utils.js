...
(function () {
    ...
    function proxyRequest(req, res, proxyTo, opts) {
        ...
        if (directProxyConfiguration) {
            var directProxyUrlObject = url.parse(directProxyConfiguration);

            if (proxyUrl.protocol === 'https:') {
                var agent = new HttpsProxyAgent({
                    proxyHost: directProxyUrlObject.hostname,
                    proxyPort: directProxyUrlObject.port
                });
                requestOptions.agent = agent;
            } else {
                requestOptions = {
                    host: directProxyUrlObject.hostname,
                    port: directProxyUrlObject.port,
                    method: req.method,
                    path: url.format(proxyUrl),
                    headers: headers
                };
            }
        }
